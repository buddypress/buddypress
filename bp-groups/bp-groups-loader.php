...
function groups_setup_nav() {
	...
	if ( bp_is_current_component( $bp->groups->slug ) && $group_id = BP_Groups_Group::group_exists($bp->current_action) ) {
...
function groups_screen_group_activity_permalink() {
	...
	if ( !bp_is_current_component( $bp->groups->slug ) || $bp->current_action != $bp->activity->slug || empty( $bp->action_variables[0] ) )
		return false;
...
function groups_screen_group_admin() {
	...
	if ( !bp_is_current_component( BP_GROUPS_SLUG ) || 'admin' != $bp->current_action )
		return false;
