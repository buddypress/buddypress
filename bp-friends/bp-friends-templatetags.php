...
function bp_friend_time_since_requested() {
	global $friends_template;
	echo bp_time_since( strtotime( $friends_template->friendship->friend->date_created ) );
}
...
function bp_add_friend_button() {
	...
	$friend_status = BP_Friends_Friendship::check_is_friend( $loggedin_userid, $current_userid );
	
	echo '<div id="friendship-button">';
	if ( $friend_status == 'pending' ) {
		_e('Friendship Requested');
	} else if ( $friend_status == 'is_friend') {
		echo '<a href="' . $loggedin_domain . $bp_friends_slug . '/add-friend/' . $current_userid . '" title="' . __('Cancel Friendship') . '" id="addremove-friend" rel="remove" class="remove">' . __('Cancel Friendship') . '</a>';
	} else {
		echo '<a href="' . $loggedin_domain . $bp_friends_slug . '/add-friend/' . $current_userid . '" title="' . __('Add Friend') . '" id="addremove-friend" rel="add">' . __('Add Friend') . '</a>';
	}
	echo '</div>';
