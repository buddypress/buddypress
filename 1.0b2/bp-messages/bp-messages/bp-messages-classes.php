...
Class BP_Messages_Thread { 
			...
			if ( $this->has_access ) {
				...
				if ( !$this->get_all_messages ) {
					...
					foreach ( $this->message_ids as $key => $message_id ) {
						if ( $this->box == 'sentbox' ) {
							if ( !messages_is_user_sender( $bp['loggedin_userid'], $message_id ) ) {
	...
	function get_unread() {
		...
		$sql = $wpdb->prepare( "SELECT unread_count FROM " . $bp['messages']['table_name_recipients'] . " WHERE thread_id = %d AND user_id = %d", $this->thread_id, $bp['loggedin_userid'] );
	...
	function get_recipients() {
		...
		for ( $i = 0; $i < count($recipients); $i++ ) {
			...
			if ( count($recipients) > 1 && $recipient != $bp['loggedin_userid'] )
	...
	function mark_as_read( $thread_id ) {
		...
		$sql = $wpdb->prepare( "UPDATE " . $bp['messages']['table_name_recipients'] . " SET unread_count = 0 WHERE user_id = %d AND thread_id = %d", $bp['loggedin_userid'], $thread_id );
		$wpdb->query($sql);
	}
	...
	function mark_as_unread( $thread_id ) {
		...
		$sql = $wpdb->prepare( "UPDATE " . $bp['messages']['table_name_recipients'] . " SET unread_count = 1 WHERE user_id = %d AND thread_id = %d", $bp['loggedin_userid'], $thread_id );
		$wpdb->query($sql);
	}
	...
	function user_is_sender($thread_id) {
		...
		return in_array( $bp['loggedin_userid'], $sender_ids );
	}
	...
	function check_access($id) {
		...
		$sql = $wpdb->prepare("SELECT id FROM " . $bp['messages']['table_name_recipients'] . " WHERE thread_id = %d AND user_id = %d", $id, $bp['loggedin_userid'] );
...
Class BP_Messages_Message {
	...
	function bp_messages_message( $id = null ) {
		...
		$this->date_sent = time();
		$this->sender_id = $bp['loggedin_userid'];
	...
	function send() {	
		...
		if ( $this->thread_id ) {
			...
			for ( $i = 0; $i < count($this->recipients); $i++ ) {
				if ( $this->recipients[$i]->user_id != $bp['loggedin_userid'] )
			...
			$serialized_sender_id = serialize( array( (int)$bp['loggedin_userid'] ) );
	...
	function get_last_sent_for_user( $thread_id ) {
		...
		$sql = $wpdb->prepare( "SELECT id FROM " . $bp['messages']['table_name_messages'] . "  WHERE sender_id = %d AND id IN (" . $wpdb->escape($message_ids) . ") ORDER BY date_sent DESC LIMIT 1", $bp['loggedin_userid'] );
...
Class BP_Messages_Notice {
	...
	function save() {
		...
		update_usermeta( $bp['loggedin_userid'], 'last_activity', date( 'Y-m-d H:i:s' ) ); 
