...
function xprofile_add_css() {
	if ( $_SERVER['SCRIPT_NAME'] == '/wp-signup.php' )
		wp_enqueue_style( 'bp-xprofile-signup', site_url( MUPLUGINDIR . '/bp-xprofile/css/signup.css' ) );	
...
function xprofile_add_structure_css() {
	...
	wp_enqueue_style( 'bp-xprofile-structure', site_url( MUPLUGINDIR . '/bp-xprofile/css/structure.css' ) );	
}
add_action( 'bp_styles', 'xprofile_add_structure_css' );

function xprofile_add_admin_css() {
	if ( strpos( $_GET['page'], 'xprofile' ) !== false ) {
		echo '<link rel="stylesheet" href="' . site_url( MUPLUGINDIR . '/bp-xprofile/css/admin.css' ) . '" type="text/css" />';
	}
}
...
function xprofile_add_admin_js() {
	if ( strpos( $_GET['page'], 'xprofile' ) !== false )
		echo '<script type="text/javascript" src="' . site_url( MUPLUGINDIR . '/bp-xprofile/js/admin.js' ) . '"></script>';
}
