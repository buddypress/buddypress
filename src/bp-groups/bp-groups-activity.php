...
function bp_groups_format_activity_action_created_group( $action, $activity ) {
	...
	$group_link = '<a href="' . esc_url( bp_groups_get_group_url( $group ) ) . '">' . esc_html( $group->name ) . '</a>';
...
function bp_groups_membership_accepted_add_activity( $user_id, $group_id ) {
	 ...
	 */
	$action = apply_filters_ref_array( 'groups_activity_membership_accepted_action', array( sprintf( __( '%1$s joined the group %2$s', 'buddypress' ), bp_core_get_userlink( $user_id ), '<a href="' . bp_groups_get_group_url( $group ) . '">' . esc_attr( $group->name ) . '</a>' ), $user_id, &$group ) );
