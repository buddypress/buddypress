...
function bp_groups_has_directory() {
	...
	return ! empty( $bp->pages->groups->id );
}
...
function bp_get_group_by( $field, $value ) {
	...
	$group = groups_get_group( (int) $group_id );
...
function groups_join_group( $group, $user_id = 0 ) {
	...
	if ( groups_check_for_membership_request( $user_id, $group_id ) ) {
		groups_delete_membership_request( 0, $user_id, $group_id );
	}
...
function groups_delete_membership_request( $membership_id, $user_id = 0, $group_id = 0 ) {
	...
	return $invites_class->delete_requests(
		array(
			'user_id' => $user_id,
			'item_id' => $group_id,
		)
...
function bp_groups_memberships_personal_data_exporter( $email_address, $page ) {
	...
	$memberships = BP_Groups_Member::get_user_memberships(
		$user->ID,
		array(
			'type'     => 'membership',
			'page'     => $page,
 ...
 * @param bool   $built_in True to only get builtin screens. False otherwise.
...
function bp_groups_get_path_chunks( $chunks = array(), $context = 'read' ) {
	...
	if ( $chunks ) {
		...
		foreach ( $chunks as $chunk ) {
			...
			} elseif ( isset( $group_screens[ $chunk ]['rewrite_id'] ) ) {
