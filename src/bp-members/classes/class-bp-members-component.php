...
class BP_Members_Component extends BP_Component {
	 ...
	 * @since 12.0.0
	 *
	 * @param WP_Query $query Required. See BP_Component::parse_query() for description.
	 */
	public function parse_query( $query ) {
		...
		/**
		 * Filter the request used to parse the Members component query.
		 *
		 * @since 15.0.0
		 *
		 * @param string $request The WP request string.
		 */
		...
		if ( isset( $member_data['object'] ) && $member_data['object'] ) {
			bp_reset_query( trailingslashit( $this->root_slug ) . $request, $query );
		...
		if ( $is_members_component ) {
			...
			if ( $member_slug ) {
				 ...
				 *
				 * E.g: example.com/ADMIN (when root profiles is enabled)
				 ...
				 * @since 2.6.0
				 *
				 * @param string $member_slug The member slug from the URI.
				 */
