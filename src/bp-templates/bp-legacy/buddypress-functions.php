	...
	switch ( $request_type ) {
		...
		case 'request_membership' :
			...
			if ( ! groups_send_membership_request( [ 'user_id' => bp_loggedin_user_id(), 'group_id' => $group->id ] ) ) {
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="group-button disabled pending membership-requested" rel="membership-requested" href="' . bp_groups_get_group_url( $group ) . '">' . __( 'Request Sent', 'buddypress' ) . '</a>';
			}
		break;
