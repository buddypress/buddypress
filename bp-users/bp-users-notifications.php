...
function bp_users_add_notification( $item_id, $user_id, $component_name, $component_action, $secondary_item_id = 0, $date_notified = false ) {
	global $bp;
...
function bp_users_delete_notification( $id ) {
	if ( !bp_users_check_notification_access( $bp->loggedin_user->id, $id ) )
		return false;

	return BP_Core_Notification::delete( $id );
}

function bp_users_get_notification( $id ) {
	return new BP_Core_Notification( $id );
}

function bp_users_get_notifications_for_user( $user_id ) {
	global $bp;
	...
	foreach ( (array)$grouped_notifications as $component_name => $action_arrays ) {
		...
		foreach ( (array)$action_arrays as $component_action_name => $component_action_items ) {
			...
			if ( function_exists( $bp->{$component_name}->notification_callback ) ) {
...
function bp_users_delete_notifications_by_type( $user_id, $component_name, $component_action ) {
	return BP_Core_Notification::delete_for_user_by_type( $user_id, $component_name, $component_action );
}

function bp_users_delete_notifications_for_user_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
	return BP_Core_Notification::delete_for_user_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id );
}

function bp_users_delete_all_notifications_by_type( $item_id, $component_name, $component_action = false, $secondary_item_id = false ) {
	return BP_Core_Notification::delete_all_by_type( $item_id, $component_name, $component_action, $secondary_item_id );
}

function bp_users_delete_notifications_from_user( $user_id, $component_name, $component_action ) {
	return BP_Core_Notification::delete_from_user_by_type( $user_id, $component_name, $component_action );
}

function bp_users_check_notification_access( $user_id, $notification_id ) {
