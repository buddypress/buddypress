...
class BP_Tests_Groups_Activity extends BP_UnitTestCase {
	...
	public function test_bp_groups_format_activity_action_created_group() {
		...
		$expected = sprintf( __( '%s created the group %s', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $g_obj ) ) . '">' . $g_obj->name . '</a>' );
	...
	public function test_bp_groups_format_activity_action_joined_group() {
		...
		$expected = sprintf( __( '%s joined the group %s', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $g_obj ) ) . '">' . $g_obj->name . '</a>' );
	...
	public function test_bp_groups_format_activity_action_group_details_updated_with_updated_name() {
		...
		$expected = sprintf( esc_html__( '%s changed the name of the group %s from "%s" to "%s"', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $group ) ) . '">Foo</a>', $group->name, 'Foo' );
	...
	public function test_bp_groups_format_activity_action_group_details_updated_with_updated_description() {
		...
		$expected = sprintf( esc_html__( '%s changed the description of the group %s from "%s" to "%s"', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $group ) ) . '">' . $group->name . '</a>', $group->description, 'Bar' );
		$this->assertSame( $expected, $a['activities'][0]->action );
	...
	public function test_bp_groups_format_activity_action_group_details_updated_with_updated_slug() {
		...
		$expected = sprintf( __( '%s changed the permalink of the group %s.', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $new_group_details ) ) . '">' . $group->name . '</a>' );
		$this->assertSame( $expected, $a['activities'][0]->action );
	...
	public function test_bp_groups_format_activity_action_group_details_updated_with_updated_name_and_description() {
		...
		$expected = sprintf( __( '%s changed the name and description of the group %s', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $group ) ) . '">Foo</a>' );
		$this->assertSame( $expected, $a['activities'][0]->action );
	...
	public function test_bp_groups_format_activity_action_group_activity_update() {
		...
		$expected = sprintf( esc_html__( '%1$s posted an update in the group %2$s', 'buddypress' ), bp_core_get_userlink( $u ),  '<a href="' . esc_url( bp_groups_get_group_url( $g_obj ) ) . '">' . esc_html( $g_obj->name ) . '</a>' );
